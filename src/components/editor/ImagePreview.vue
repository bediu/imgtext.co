<template>
  ...
  id="imgCanvas" 
  class="canvas" 
  :class="{'canvas-anim' : this.$store.state.imageFile.state,
           'canvas-drag' : this.$store.state.imageFile.draggable}" 
           
           @mousedown="canvasDragStart"
           @mouseup="canvasDragStop" 
           @mouseout="canvasDragStop" 
      ...
      canvasDrag(e) {
        if (this.$store.state.imageFile.draggable && this.isDragging) {
          ...
          if (this.dragContext) {
            ...
            this.$store.state.canvasContext.drawImage(this.$store.state.imageFile.raw, 0, 0);
