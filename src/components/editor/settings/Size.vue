<template>
    <div class="btn-group btns-size" :class="{'btn-group-dis' : !this.$store.state.imageFile.state}">
        <!-- fit to editor -->
        <button class="btn-i" @click="fit" :disabled="!this.$store.state.imageFile.state">
        ...
        <!-- 100% -->
        <button class="btn-i btn-t" @click="realSize" :disabled="!this.$store.state.imageFile.state">
            100%
        </button>
    </div>
</template>
